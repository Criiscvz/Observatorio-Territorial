...
export class DatasetViewComponent implements OnInit {
  ...
  readonly chartTypes: ChartType[] = [
    // === UNIVARIABLES ===
    {
      ...
      icon: 'bar_chart',
      description: 'Comparar frecuencias por categoría',
    ...
    },
    {
      ...
      name: 'Pastel (Torta)',
      icon: 'pie_chart',
      description: 'Mostrar proporciones del total',
      forTypes: ['CATEGORICO'],
    },
    {
      id: 'donut',
      name: 'Anillo (Donut)',
      icon: 'donut_large',
      description: 'Proporciones con total central',
      forTypes: ['CATEGORICO'],
    },
    {
      id: 'histogram',
    ...
    },
    {
      ...
      name: 'Área',
      icon: 'area_chart',
      description: 'Tendencias con área sombreada',
      forTypes: ['FECHA', 'NUMERICO', 'CATEGORICO'],
    },
    {
      id: 'funnel',
      name: 'Embudo',
      icon: 'filter_list',
      description: 'Procesos secuenciales',
      forTypes: ['CATEGORICO'],
    },
    {
      id: 'treemap',
      name: 'Treemap',
      icon: 'grid_view',
      description: 'Jerarquía por tamaño',
      forTypes: ['CATEGORICO'],
    },
    {
      id: 'gauge',
      name: 'Indicador',
      icon: 'speed',
      description: 'Medidor de progreso',
      forTypes: ['NUMERICO'],
    },
    {
      id: 'radar',
      name: 'Radar',
      icon: 'radar',
      description: 'Comparar múltiples categorías',
      forTypes: ['CATEGORICO'],
    },
    // === BIVARIABLES ===
    {
      id: 'scatter',
      name: 'Dispersión (Scatter)',
      icon: 'scatter_plot',
      description: 'Correlación entre 2 numéricas',
      forTypes: ['NUMERICO'],
      bivariable: true,
    },
    {
      ...
      description: 'Comparar promedios por categoría',
    ...
    },
    {
      ...
      name: 'Mapa de Calor',
      icon: 'grid_on',
      description: 'Matriz de frecuencias cruzadas',
      forTypes: ['CATEGORICO', 'TEXTO'],
      bivariable: true,
    },
    {
      ...
      icon: 'leaderboard',
      description: 'Promedio numérico por categoría',
      forTypes: ['CATEGORICO', 'NUMERICO', 'TEXTO'],
      bivariable: true,
    },
    {
      ...
      name: 'Serie Temporal',
      icon: 'timeline',
      description: 'Evolución de valores en el tiempo',
    ...
    },
    {
      ...
      icon: 'stacked_bar_chart',
      description: 'Evolución de categorías en el tiempo',
    ...
    switch (type) {
        ...
        return {
          ...
          tooltip: {
            ...
            formatter: (params: any) => {
              ...
              return (
                `<div style="font-weight:600;margin-bottom:8px;color:${colors[0]}">${variableName}</div>` +
      ...
      case 'pie':
        return {
          title: {
            ...titleStyle,
            subtext: `${labels.length} categorías | ${total.toLocaleString()} registros`,
          },
          tooltip: {
            ...tooltipStyle,
      ...
      case 'donut':
        return {
          ...
          tooltip: {
            ...
            formatter: (params: any) => {
              return (
                `<div style="font-weight:600;margin-bottom:8px;color:#EC4899">${variableName}</div>` +
                `<div style="display:flex;align-items:center;gap:8px;">` +
                `<span style="width:14px;height:14px;background:${params.color};border-radius:50%;"></span>` +
                `<span style="font-weight:500">${params.name}</span></div>` +
                `<div style="font-size:22px;font-weight:700;margin:8px 0;">${params.value.toLocaleString()}</div>` +
          ...
          graphic: [
            ...
            },
            {
              ...
              left: '28%',
              top: '52%',
              style: { text: 'Total', fontSize: 14, fill: '#6B7280' },
            },
          ],
          series: [
            {
      ...
      case 'histogram':
        return {
          title: {
            ...
            subtext: `${values.length} rangos | ${total.toLocaleString()} registros`,
          },
          tooltip: {
            ...tooltipStyle,
            trigger: 'axis' as const,
            ...
            formatter: (params: any) => {
              ...
              const percent = total > 0 ? ((p.value / total) * 100).toFixed(1) : 0;
              return (
                `<div style="font-weight:600;margin-bottom:8px;color:#14B8A6">${variableName}</div>` +
      ...
      case 'area':
        ...
        return {
          title: {
            ...titleStyle,
            text: `Tendencia de ${variableName}`,
            subtext: `${labels.length} puntos de datos`,
          },
          tooltip: {
            ...tooltipStyle,
            trigger: 'axis' as const,
            formatter: (params: any) => {
              const p = Array.isArray(params) ? params[0] : params;
              return (
                `<div style="font-weight:600;margin-bottom:8px;color:#F59E0B">${variableName}</div>` +
        ...
        return {
          ...
          tooltip: {
            ...
            formatter: (params: any) => {
              ...
              return (
                `<div style="font-weight:600;margin-bottom:8px;color:${colors[params.dataIndex % colors.length]}">${variableName}</div>` +
      ...
      case 'treemap':
        return {
          title: {
            ...titleStyle,
            subtext: `${labels.length} categorías | ${total.toLocaleString()} total`,
          },
          tooltip: {
            ...tooltipStyle,
            formatter: (params: any) => {
              const pct = total > 0 ? ((params.value / total) * 100).toFixed(1) : 0;
              return (
                `<div style="font-weight:600;margin-bottom:8px;color:${params.color}">${variableName}</div>` +
                `<div style="font-weight:500">${params.name}</div>` +
          ...
          series: [
            {
              ...
              splitLine: {
                distance: -18,
                length: 18,
                lineStyle: { color: cfg.textColorSecondary, width: 2 },
              },
              ...
              anchor: {
                ...
                showAbove: true,
                size: 20,
                itemStyle: { borderWidth: 8, borderColor: colors[0] },
              },
              title: {
                show: true,
                offsetCenter: [0, '70%'],
  ...
  private getBivariableChartOptions(data: BivariableResponse, type: string): EChartsOption {
      ...
      return {
        ...
        tooltip: {
          ...
          formatter: (params: any) => {
            ...
            return (
              `<div style="font-weight:600;color:#6366F1;margin-bottom:8px;">${varX}</div>` +
              ...
              (count > 0
                ? `<div><span style="color:#6B7280;font-size:11px;">Registros en este periodo</span><br/>` +
    ...
    if (d.points && d.points.length > 0) {
      ...
      return {
        ...
        tooltip: {
          ...
          formatter: (p: any) => {
            return (
              `<div style="font-weight:600;color:#8B5CF6;margin-bottom:8px;">Punto de Datos</div>` +
    ...
    if (d.heatmap && d.labels_x && d.labels_y) {
      ...
      const totalCells = d.heatmap.reduce(
        (sum: number, h: [number, number, number]) => sum + h[2],
        0,
      );
      ...
      return {
        ...
        tooltip: {
          ...
          formatter: (p: any) => {
            ...
            return (
              `<div style="font-weight:600;color:#EC4899;margin-bottom:8px;">Cruce de Variables</div>` +
    ...
    if (d.series && Array.isArray(d.series) && d.labels_x) {
      ...
      const totalValues = d.series.reduce(
        (sum: number, s: { data: number[] }) =>
          sum + s.data.reduce((a: number, b: number) => a + b, 0),
        0,
      );
      ...
      return {
        ...
        tooltip: {
          ...
          formatter: (params: any) => {
            ...
            return (
              `<div style="font-weight:600;color:#14B8A6;margin-bottom:8px;">${varX}</div>` +
              `<div style="font-size:14px;font-weight:500;margin-bottom:10px;">${p.name}</div>` +
              `<div style="display:grid;gap:8px;">` +
              `<div><span style="color:#6B7280;font-size:11px;">Promedio de ${varY}</span><br/>` +
              `<span style="font-size:24px;font-weight:700;color:#14B8A6;">${p.value.toLocaleString()}</span></div>` +
              (count > 0
                ? `<div><span style="color:#6B7280;font-size:11px;">Cantidad de registros</span><br/>` +
                  `<span style="font-size:16px;font-weight:600;">${count.toLocaleString()}</span></div>`
                : '') +
        ...
        series: [
          {
              ...
              itemStyle: {
                color: {
                  ...
                  colorStops: [
                    ...
                    {
                      offset: 1,
                      color: this.adjustColor(this.colors[i % this.colors.length], -40),
                    },
                  ],
                },
            ...
            markLine: {
              ...
              data: [
                {
                  yAxis: avg,
                  label: { formatter: `Prom: ${avg.toFixed(1)}`, position: 'end' as const },
                },
              ],
            },
          },
